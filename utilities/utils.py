...
class Utilities:
    ...
    def setupForTraining(self, args, split='train', transform = None):
        # Retrieve the correct dataset
        dataset = Utilities.selectDataset(args, split, transform)

        # Wrap up the data in a PyTorch dataset loader
        data_loader = data.DataLoader(dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)

        # Setup the selected model
        if args.model == "TripletResnetSoftmax":
            #model = TripletResnet50Softmax(pretrained=True, num_classes=dataset.getNumClasses(), img_type=args.img_type)
            model = TripletResnet50Softmax(pretrained=True, num_classes=155, img_type=args.img_type)
        elif args.model == "TripletResnet":
            model = TripletResnet50(pretrained=True, embedding_size=args.embedding_size, img_type=args.img_type)
        else:
        ...
        if args.loss_function == "OnlineReciprocalTripletLoss":
        ...
        if "Reciprocal" in args.loss_function:
            ...
            optimiser = optim.SGD(model.parameters(), lr=args.learning_rate, momentum=0.9,
    ...
    @staticmethod
    def selectDataset(args, split, transform):
