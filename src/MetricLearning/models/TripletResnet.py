...
class Triplet_ResNet(nn.Module):
    def __init__(self, block, layers):
        ...
        self.fc = nn.Linear(512 * block.expansion, 1000) #defult if you want to use the pretrained weight
...
def TripletResnet50(pretrained=False, num_classes=1000, embedding_size=128, img_type="RGB"):
    model = Triplet_ResNet(Bottleneck, [3, 4, 6, 3])
    ...
    # We might need to modify the first conv layer
    if img_type == "RGBD":
        model.conv1 = nn.Conv2d(4, 64, kernel_size=7, stride=2, padding=3, bias=False)

    # Modify the last layers
    model.fc = nn.Linear(2048, 1000) #1000 # num_classes
    model.fc_embedding = nn.Linear(1000, embedding_size) ##1000 # num_classes , 128 #embedding_size
